...
async fn attach_scorpiofs_worktree(
    ...
    if let Some(previous_mountpoint) = previous_mountpoint.as_deref()
        && Path::new(previous_mountpoint) != target
    {
        state
            .entries
    ...
    let registration_epoch = state.next_epoch();
    // A ScorpioFS mount is a linked worktree like any other, and the fact
    // that one existed must OUTLIVE its entry (§C.4.3) — detach deletes the
    // entry, and the ambiguous-sidecar rules ask "did one ever exist".
    state.linked_history = LinkedHistory::Existed;
    state.entries.push(WorktreeEntry {
        path: target.to_string_lossy().into_owned(),
        is_main: false,
        locked: false,
        lock_reason: None,
        // v2 (§C.7): the mount's stable id, so `worktree repair <path>` can
        // restore a corrupt gitdir identity from the registry.
        worktree_id: Some(worktree_id.clone()),
        state: WorktreeEntryState::Active,
        // A fresh generation for every registration, so a client fenced on
        // the previous one at this path/id is refused rather than served.
        epoch: registration_epoch,
    });
...
async fn detach_scorpiofs_worktree(path: String) -> WorktreeResult<ScorpioFsDetachOutput> {
    ...
    // Detach removes the mountpoint AND the gitdir below, so the directory is
    // gone for the strict sweep's purposes — layer ownership and the sparse
    // view go with it. Instance ids are path-derived, so a later reattach at
    // this same mountpoint would otherwise inherit these rows.
    let db = crate::internal::db::get_db_conn_instance().await;
    gc_worktree_scoped_rows_strict(&db, &worktree_id, true)
        .await
        .map_err(|error| {
            WorktreeError::IoWrite(format!(
                "ScorpioFS mount was removed but the scoped rows for worktree '{worktree_id}' \
                 could not be cleared: {error}; run `libra worktree repair`"
            ))
        })?;
...
async fn rollback_scorpiofs_attach(
    ...
    // Best-effort on the rollback path: the attach already failed, and the
    // caller reports THAT error. A sweep failure here only leaves rows a
    // later attach's strict pre-seed sweep has to clear, so warn and go on.
    let db = crate::internal::db::get_db_conn_instance().await;
    if let Err(error) = gc_worktree_scoped_rows_strict(&db, worktree_id, true).await {
        tracing::warn!(
            worktree_id,
            error,
            "failed to GC per-worktree rows while rolling back a ScorpioFS attach"
        );
    }
    ...
    #[cfg(all(target_os = "linux", feature = "scorpiofs-direct"))]
    {
        ...
        if let Some(worker) = state.worker.as_ref() {
            if let Ok(client) = HttpScorpioFsClient::new(&worker.endpoint)
                && client.service_info().await.is_ok()
            {
