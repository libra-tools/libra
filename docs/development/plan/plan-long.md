...
审计时间：**2026-07-17**。发现范围严格限定为 `/Volumes/Data/competition/*/*`；未扫描内部嵌套仓库、vendor 或 submodule。13 个直接仓库本轮全部通过 clean/tracking 门禁；表中 revision 是实际审计 checkout，不把 blocked 仓库表述为最新版本。

| 竞品 | 类型 | 分支 | 审计 revision | 更新结果 | 最后审计 | 证据入口 |
|---|---|---|---|---|---|---|
| `agenta-ai/agenta` | Libra | `main` | `650d4ed` | 已是最新 | 2026-07-17 | `README.md`、`docs/`、agent session/HITL/runner 源码与测试 |
| `cursor/agent-trace` | Git | `main` | `2754f07` | 已是最新 | 2026-07-17 | `README.md`、`schemas.ts`、`reference/` |
| `entireio/cli-checkpoints` | Libra | `entire/checkpoints/v1` | `620170f` | 已是最新；本轮现场干净 | 2026-07-17 | checkpoint object tree |
| `entireio/cli` | Libra | `main` | `df765ab` | 已是最新 | 2026-07-17 | `CHANGELOG.md`、`docs/`、`api/checkpoint/`、`cmd/entire/cli/`、测试 |
| `entireio/git-sync` | Libra | `main` | `298c7f1` | 已是最新；本轮现场干净 | 2026-07-17 | `README.md`、`docs/`、`client.go`、`internal/` |
| `epicgames/lore` | Git | `main` | `370536a` | 已是最新 | 2026-07-17 | `docs/`、`lore-revision/src/`、`lore-revision/tests/` |
| `facebook/sapling` | Git | `main` | `16befcd5849` | fast-forward：`d43b86e7bb5` → `16befcd5849` | 2026-07-17 | `eden/mononoke/derived_data/`、`servers/slapi/`、`tests/integration/edenapi/` |
| `GitButler/gitbutler` | Git | `master` | `e1f599d2f4` | fast-forward：`903b12721a` → `e1f599d2f4` | 2026-07-17 | `crates/but-workspace/src/branch/move_branch.rs`、`crates/but-api/tests/api/branch_move.rs`、`crates/but/src/command/legacy/status/mod.rs` |
| `GitButler/grit` | Git | `main` | `dfb079967` | 已是最新 | 2026-07-17 | `TESTING.md`、`data/tests/`、`scripts/run-tests.sh`、`docs/progress/` |
| `jj-vcs/jj` | Git | `main` | `3efc18b54` | fast-forward：`30c5071a8` → `3efc18b54` | 2026-07-17 | `lib/src/repo.rs`、`lib/src/transaction.rs`、`lib/tests/test_view.rs` |
| `mainline/mainline` | Git | `main` | `6025c45` | 已是最新 | 2026-07-17 | `README.md`、`docs/specs/`、`internal/engine/`、测试 |
| `StepzeroLab/research-git` | Libra | `main` | `76f45b5` | 已是最新 | 2026-07-17 | `README.md`、`src/rgit/`、`tests/`、v2/v3 设计文档 |
| `xai-org/grok-build` | Git | `main` | `c68e39f` | 已是最新；本轮首次纳入 | 2026-07-17 | `README.md`、`crates/codegen/xai-grok-agent/README.md`、`xai-grok-test-support/`、`xai-grok-hooks/`、`xai-grok-pager-pty-harness/` |
...
**本次结论：无长期优先级变化。** GitButler 新增单分支 stack 重排、无副作用 dry-run preview、materialize 后 order/HEAD 持久化，并把 Change ID 提升为默认 status 主身份，强化 LR-02/LR-03/LR-04，且其 Forge capability 缺失时显式 gate 查询继续支持 LR-08；Jujutsu 把 `merge_operation()` 与 `rebase_descendants()` 从隐式耦合改为调用方显式组合，强化 LR-02 与 LR-05 的边界。Sapling 本轮是服务端 augmented manifest 派生/路由与 flaky test 修复，不构成 Libra 客户端长期优先级变化。首次纳入的 Grok Build 提供可移植 Agent 定义、hermetic 子进程、协议 fault injection 和 PTY resume 场景，强化 SB-04 与 `plan-20260715.md` 的测试/运行时门禁；Libra 已有 Goal/skills/多 Agent 与 Code harness 基础，缺口是统一可靠性设施而不是新的 VCS LR。其 `PreToolUse` hook failure 默认 fail-open 依赖受保护环境威胁模型，不适合作为 Libra 安全边界的通用默认。
...
以下事实已在 2026-07-17 以当前 Libra checkout 的源码、测试与兼容文档复核；历史计划不作为实现证据：
...
|---|---|---:|---|---|---|---|---|
| LR-01 | 完整多工作区隔离与并行 Agent 工作区 | P0 | 已验证 | private HEAD/index/reflog 已实现；sequencer、pseudo-ref、Agent lease 和 parallel lanes 未完成 | Lore `370536a` `lore-revision/src/state.rs`、`tests/node_add_parallel.rs`；GitButler `e1f599d2f4` `crates/but-workspace/` | 无；`plan-20260708.md` 仅覆盖相邻 Git compatibility | 2026-07-17 |
| LR-02 | 全命令 Operation Log、完整快照与 Undo/Redo | P0 | 已验证 | `op log/show/restore` 存在，但仅少量 mutation 接入且 snapshot 不含 index/worktree/sequencer | Jujutsu `3efc18b54` `lib/src/repo.rs`、`lib/src/transaction.rs`、`lib/tests/test_view.rs`；GitButler `e1f599d2f4` branch dry-run/oplog | 无 | 2026-07-17 |
| LR-03 | 稳定 Change ID 与历史重写谱系 | P0 | 已验证 | Libra 无 stable change identity 或持久 lineage；review/intent/Forge 仍依赖 commit/session 身份 | GitButler `e1f599d2f4` `crates/but-core/src/change_id.rs`、legacy status、Forge；Jujutsu evolution predecessor tests | 无 | 2026-07-17 |
| LR-04 | 非交互 Hunk API、Hunk 归属与 Stack 编辑 | P0 | 已验证 | `diff --json` 可读 hunks；稳定 ID、assignment、hunk mutation 与 stack rewrite 缺失 | GitButler `e1f599d2f4` `crates/but-workspace/src/branch/move_branch.rs`、branch-move tests；Sapling `16befcd5849` `eden/scm/lib/linelog/` | 无；`plan-20260708.md` 明确保留 D15 延后 | 2026-07-17 |
| LR-05 | 一等冲突对象与 Modeless Sequencer | P1 | 已验证 | Git-compatible conflict 基础较完整；versioned conflict object、record-conflicts 和 descendant rebase 缺失 | Jujutsu `3efc18b54` operation merge/rebase boundary；Sapling `16befcd5849` linelog/stacks | 无 | 2026-07-17 |
| LR-06 | Intent Seal、Intent-Commit Pin 与安全团队发布 | P1 | 已验证 | 本地 Intent/Decision/checkpoint 已有；seal、stable pin 与白名单团队 publication 缺失 | Mainline `6025c45` `internal/engine/seal.go`、`merge.go`、property tests | `plan-20260713.md` 覆盖 capture/coverage 基础，不覆盖 seal/publication | 2026-07-17 |
| LR-07 | 开工前意图检索与语义冲突 Preflight | P1 | 已验证 | skill/session/context 基础存在；团队 intent projection、确定性 overlap receipt 与 pre-edit gate 缺失 | Mainline `6025c45` `internal/engine/preflight.go`、`context_retrieval.go`、tests | `plan-20260713.md` 覆盖 transcript import/coverage；`plan-20260715.md` 覆盖 Code runtime/UI，均非本 LR 完成计划 | 2026-07-17 |
| LR-08 | Forge/PR/CI 集成与 Stacked Review | P1 | 已验证 | remote/auth/open/local Agent review 已有；Forge trait、PR/CI 状态与 stacked mapping 缺失 | GitButler `e1f599d2f4` `crates/but-forge/`、Forge capability gates、workspace projection | 无 | 2026-07-17 |
| LR-09 | Materializing Sparse Checkout、Partial Clone 与 VFS Hydration | P2 | 已验证 | sparse-view、explicit hydrate、alternates/tiered storage 已有；materialization、promisor 和 transparent VFS 缺失 | Lore `370536a` `lore-revision/src/projfs/`、parallel state tests；Sapling `16befcd5849` `eden/fs/` | 无；`plan-20260708.md` 保留 D10/D18 延后 | 2026-07-17 |
| LR-10 | Feature/Research Capsule 与实验谱系 | P2 | 已验证 | Agent artifacts/semantic extraction 可复用；capsule lifecycle、run lineage、compare/ablation 缺失 | research-git `76f45b5` `src/rgit/{curation,runner,recall,provenance}.py`、`tests/test_e2e.py` | 无；`plan-20260713.md` 仅提供 capture source，非 capsule 实施 | 2026-07-17 |
...
- Grok Build `c68e39f` 的 `xai-grok-test-support` 把 hermetic process spawn、`kill_on_drop`、协议帧级 drop/delay/duplicate/sever fault injection 和 PTY resume 场景做成共享设施；Libra 已在 review launcher、provider mocks、Code PTY harness 和若干 `env_clear` fixture 中分别具备部分能力，但尚未形成同等统一的资源/故障注入层。该证据强化本 SB，不新增产品 LR。
...
- **竞品证据**：Lore `370536a` 的 `lore-revision/src/state.rs` 与 `lore-revision/tests/node_add_parallel.rs` 把并发 sibling publish、parallel staging 和 tree corruption 回归作为存储正确性问题；GitButler `e1f599d2f4` 的 `crates/but-workspace/` 继续维护 workspace state projection，本轮单分支 stack reorder 也证明 logical lane 仍依赖独立 projection/materialization，不能替代 per-worktree mutable-state 隔离。
...
- **竞品证据**：Jujutsu `3efc18b54` 的 `lib/src/transaction.rs`、`lib/src/repo.rs` 与 `lib/tests/test_view.rs` 把 operation view merge 和 `rebase_descendants()` 改为调用方显式组合，避免底层 merge 隐式附带 rewrite；GitButler `e1f599d2f4` 的 `crates/but-api/src/branch.rs` 与 branch-move tests 把 dry-run preview、oplog snapshot 和真正 materialize/persist 分开。
...
- **竞品证据**：GitButler `e1f599d2f4` 的 `crates/but-core/src/change_id.rs` 提供稳定 ID；本轮 `crates/but/src/command/legacy/status/mod.rs` 进一步把 Change ID 设为默认 status 主标识，commit SHA 退为辅助提示，并继续用于 undo/redo、agent target 和 Forge association；Jujutsu 的 evolution predecessor tests 记录 rewrite predecessor 与 operation。
...
- **竞品证据**：GitButler `e1f599d2f4` 的 `crates/but-workspace/src/branch/move_branch.rs`、`crates/but-api/src/branch.rs` 与两层 branch-move tests 新增单分支 stack 重排：dry-run 返回 overlay preview 且不写 order/oplog/HEAD，成功路径才 materialize、持久化 order 并移动 tip；既有 diff/workspace/status detail 测试继续覆盖 file/hunk selection、move/discard/commit。Sapling `16befcd5849` 的 `eden/scm/lib/linelog/` 与 stacks 仍提供可重写提交栈基础，本轮 augmented-manifest 服务端变化不改变本 LR 判断。
...
- **竞品证据**：Jujutsu `3efc18b54` 的 operation/view merge tests 展示冲突与并发 operation 可作为图状态继续合并；本轮将 `rebase_descendants()` 移到明确调用点，说明 descendant rewrite 应是可观察、可选择的上层步骤，而非 operation merge 的隐藏副作用。Sapling `16befcd5849` 的 linelog/stacks 继续强化 descendant rewrite 的工程价值。
...
- **竞品证据**：GitButler `e1f599d2f4` 将 PR association 从持久 PR-number 写路径收敛为 Forge cache 派生 projection，见 `crates/but-forge/association.rs`、`crates/but-api/tests/api/forge_pr_association.rs` 和 workspace enrichment；本轮 `apps/lite/ui/src/api/queries.ts` 进一步要求 PR/check 查询按 provider capability 显式 gate，且默认 status 继续优先 Change ID。
...
- **竞品证据**：Lore `370536a` 的 `lore-revision/src/projfs/`、parallel state/staging tests 与 corruption fixes 证明 VFS 之前必须先解决并发 tree mutation 正确性；Sapling `16befcd5849` 的 `eden/fs/` 继续提供大仓库按需读取参照。本轮 Sapling augmented-manifest 派生/路由只强化服务端 derived-data 能力，没有证明 Libra 应跳过 materializing sparse/promisor 基础直接建设同类服务。
...
本次复核未发现新增日期计划，也没有代码/测试证据足以改变既有映射：`plan-20260713.md` 仍只明确 M1-M3 已实现、M4-M6 未声明完成；`plan-20260714.md` 仍是已通过评审但未声明实现的规格；`plan-20260715.md` 仍明确“只规划后续实现”。因此 LR 状态与链接保持不变。
