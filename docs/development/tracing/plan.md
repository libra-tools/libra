...
| 2026-07-05 | Task A8.5（AG-24a 合规） | 进行中（session 362f0f7a 已认领） | — | — | in-progress 认领标记：避免并行 session 重复实施 A8.5。范围按任务卡：`agent_audit_log` append-only 迁移（拒 UPDATE/DELETE，_down 只停写不删数据）、checkpoint show/export raw 访问 `--allow-raw` 门禁+audit 记录（redacted --detail/--transcript 不需要）、`libra agent clean --gc` 实现 retention 窗口（transcript_days=90/stderr_days=30，不碰 audit_log）、本地 erasure 三面一致（重写 refs/libra/traces + 删 agent_session/checkpoint 行 + 清 object_index）、retention/max_transcript_read_bytes settings 键+校验+文档、新错误码。findings-GC 子项前置 A7/A8 未动工→按验收条款 deferred（A9 release notes 说明）。与并发 A7（session d59de36b）文件重叠：checkpoint.rs/error.rs/error-codes.md/Cargo.toml/INDEX.md——additive 编辑、提交前重查、显式 add 避免收编 A7。完成后本行改写为验收记录 |
