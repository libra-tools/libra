...
| 合规性与标准符合性 | ✅ 合理 | retention、erasure、raw export 授权、append-only audit、redaction report、本地删除一致性与 cloud mirror 条件边界均已固化为可执行规格；方案不再把 redaction 误当作完整合规。 | transcript retention、删除权、raw export 授权、audit log 不可变性和 redaction report 已在本文固化；普通 checkpoint retention 已传播 D1 prune fence 并删除远端 capture catalog 行。session erasure tombstone/catalog 删除和 R2 物理删除仍待建。 | 落地 retention / GC、删除一致性矩阵、redaction report、raw `--allow-raw` 显式授权；audit log 必须 append-only，字段包含 actor、scope、raw/detail 标志、redaction summary、object ids、reason；session erasure 传播落地前，文档必须写明 `cloud restore` 会复活已本地擦除 session，不得把 checkpoint catalog fence 误写成 R2 物理删除。 |
...
| 合规性与标准符合性 | 满足。retention、erasure、raw export、audit、redaction report、本地/cloud 条件边界已成可执行规格。 | GC/erasure 测试证明重写 `refs/libra/traces` + 删除 DB/object_index；raw export 必写 append-only audit；redaction report 不含原文；普通 checkpoint retention 的 D1 prune fence/catalog 删除已实现，session erasure 与 R2 物理删除仍 deferred，release notes 必须写明 `cloud restore` 会复活已本地擦除 session。 |
...
| T10 | 已擦除 session 被 cloud restore 复活 | local GC / cloud sync / restore | **现存缺口**：普通 checkpoint retention 已有 D1 prune fence/catalog 删除，但 session erasure 不传播 tombstone 或 catalog 删除，故 restore 会复活已擦除 session；R2 物理删除同样 deferred | 本地 erasure 重写 ref + DB/object_index 删除；**待建**：session erasure 传播 D1 tombstone/catalog 删除与 R2 delete，restore 尊重 tombstone | 现阶段文档必须明确 restore 会复活已本地擦除 session；不得把已实现的 checkpoint catalog fence 表述成 session erasure 或 R2 物理删除已覆盖 |
...
| cloud session-erasure tombstone / R2 physical deletion | deferred（mirror 与普通 checkpoint D1 prune fence/catalog 删除已生效） | local session erasure 仍执行，但不传播 tombstone/catalog 删除到 D1，且不物理删除 R2 payload | session erasure 传播落地前：restore/re-sync 会复活已本地擦除 session，release notes 必须写明此限制 |
...
| 数据模型 | cloud mirror tombstone propagation for agent capture data | D1/R2 agent capture mirror **已实现并默认生效**：`libra cloud sync` 每次经 `sync_agent_capture_tables`（`src/command/cloud.rs`）把 `agent_session`/`agent_checkpoint` upsert 到 D1，`libra cloud restore` 经 `restore_agent_capture_from_d1` 读回；普通 checkpoint retention 还会传播 D1 prune tombstone 并删除远端 capture catalog 行。M4 已让本地 `erase_session_local` 写 `agent_import_tombstone` 并 fence 本地 import/export/coverage writer；**deferred 的 delete/tombstone propagation 仅指 session erasure tombstone/catalog 删除与 R2 物理删除**：本地 session erase 不删除 D1 mirror 行，tombstone 也不上传，因此 `libra cloud restore` 或跨机 re-sync 仍可**复活已本地擦除的 session**。**A0-10 文档守卫已收口**：`compat_agent_docs_contract::agent_doc_declares_cloud_tombstone_deferred` 钉住「mirror 与普通 checkpoint prune 已生效、session erasure propagation deferred、restore 可复活 session」措辞不可漂移；`command_test::agent_erasure_local_tombstone` 证明本地 erase 幂等且不触碰 cloud；L3 `agent_cloud_tombstone_test` 在真实 D1 上证明本地 erase 后 D1 mirror 行仍在。 | AG-24a / M4 local barrier + future cloud tombstone |
