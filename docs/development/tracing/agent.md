...
- **scope 三类**：`committed`（TurnEnd / SessionEnd，已实现）、`subagent`（SubagentStart / End，**A0-02 已实现**——每个 subagent 边界物化一个独立 `scope='subagent'` checkpoint，携带 `parent_checkpoint_id` 指向所属 session 最近的 committed checkpoint）、`temporary`（ToolUse 等，仍为规划）。`temporary` 的虚线箭头表示规划路径，尚未在当前 hook ingest 中落地。
...
| `opencode` | `OpenCode` / `opencode` | first-batch supported；hook-installable | hook install/uninstall/status、lifecycle ingest、transcript read、checkpoint/export、review evidence | 以上游实测为准：当前写入 `.opencode/plugins/` Libra 插件文件并保留用户配置；单数 `.opencode/plugin/` 只可作为上游兼容读取/迁移输入，不作为 Libra 新写入目标 |
...
| 合规性与标准符合性 | ✅ 合理 | retention、erasure、raw export 授权、append-only audit、redaction report、本地删除一致性与 cloud mirror 条件边界均已固化为可执行规格；方案不再把 redaction 误当作完整合规。 | transcript retention、删除权、raw export 授权、audit log 不可变性和 redaction report 已在本文固化；cloud restore/delete 一致性分为两层：本地 `refs/libra/traces` / object / DB 必须立即一致，而 D1/R2 mirror 虽已生效（`cloud sync`/`restore`），其**删除/tombstone 传播**仍是待建的强制实现面。 | 落地 retention / GC、删除一致性矩阵、redaction report、raw `--allow-raw` 显式授权；audit log 必须 append-only，字段包含 actor、scope、raw/detail 标志、redaction summary、object ids、reason；delete/tombstone 传播落地前，文档必须写明 `cloud restore` 会复活已本地擦除对象，不得声称 D1/R2 删除已覆盖。 |
...
| 合规性与标准符合性 | 满足。retention、erasure、raw export、audit、redaction report、本地/cloud 条件边界已成可执行规格。 | GC/erasure 测试证明重写 `refs/libra/traces` + 删除 DB/object_index；raw export 必写 append-only audit；redaction report 不含原文；D1/R2 mirror 已生效但删除/tombstone 传播未实现，release notes 必须写明 `cloud restore` 会复活已本地擦除 capture。 |
...
3. `agent_session` / `agent_checkpoint` 的 D1 mirror **已默认生效**（`libra cloud sync` upsert，`libra cloud restore` 读回；`agent_usage_stats` 仍无 mirror）。deferred 的是**删除/tombstone 传播**：本地 erasure 不删除 D1/R2 mirror 行，restore/re-sync 可复活已擦除 capture。本文所有涉及 cloud **删除传播/tombstone** 的合规要求描述的是这个待建缺口，不得被误读为已覆盖。
...
7. **合规与保留策略（可执行门禁，非口号）**：外部 transcript、prompt、context、stderr、review findings 都按潜在 PII 处理。AG-24a 必须落地实现面，AG-24 负责同步文档、compat 守卫与 release notes：
   ...
   - **删除一致性矩阵**：`refs/libra/traces` 是 GC root（`history.rs`），单删对象不可达；删除/被遗忘权（erasure）必须**重写 traces ref**（catalog 重建剔除目标 checkpoint）+ 删 `agent_checkpoint`/`agent_session` 行 + 删 `object_index`。**D1/R2 mirror 已存在**（`cloud sync`/`restore` 已生效），但 cloud delete/tombstone 传播**未实现**：本地 erasure 只保证本地对象/SQLite/ref 删除一致性，D1/R2 mirror 行不被删除，`cloud restore` 会复活；文档不得声称 D1/R2 删除已覆盖，必须写明这一复活限制。
...
| T10 | 已删除 transcript/checkpoint 被 cloud restore 复活 | local GC / cloud sync / restore | **现存缺口**：`agent_session`/`agent_checkpoint` 已有 D1 mirror（`cloud sync`/`restore` 已生效），但本地 erasure 不传播删除，故 restore 会复活已擦除 capture | 本地 erasure 重写 ref + DB/object_index 删除；**待建**：erasure 传播 D1/R2 delete tombstone，restore 尊重 tombstone | 现阶段文档必须明确 restore 会复活已本地擦除 capture（tombstone 传播落地前的已知限制）；不得声明 cloud 删除已覆盖 |
...
| AG-20 checkpoint/export/doctor/pagination | 否 | 否 | 本地对象/SQLite/ref 一致性先落地；D1 mirror 已生效（`cloud sync`/`restore`），删除/tombstone 传播为后续扩展 |
...
| AG-24a retention/erasure/audit 实现 | 否 | 否 | 本地对象/SQLite/ref/audit 一致性先落地；D1 mirror 已生效，cloud tombstone 传播是待建强制面 |
...
3. `agent_session` / `agent_checkpoint` / `agent_usage_stats` 无 SeaORM entity（走 raw SQL）。**D1 mirror 现状**：`agent_session`/`agent_checkpoint` **已有** D1 mirror 表与同步（`D1Client::{ensure,upsert,list}_agent_{session,checkpoint}` + `libra cloud sync`/`restore`，见 `src/command/cloud.rs`）；`agent_usage_stats` 仍无 mirror。扩字段时同步写 raw SQL migration、D1 mirror schema、测试和本文表格。**已知缺口**：本地删除（erasure/GC）**不**传播到 D1/R2——删除/tombstone 传播未实现，release notes 必须写明 `libra cloud restore` 会复活已本地擦除的 capture，直到 tombstone 传播落地。
...
| 标准化 `EventType`：SessionStart/TurnStart/TurnEnd/Compaction/SessionEnd/**SubagentStart/SubagentEnd**/ModelUpdate/ToolUse（9 个） | `LifecycleEventKind` 已有 13 个变体并包含 `SubagentStart`/`SubagentEnd`；Codex provider parser/settings 已映射原生 subagent hooks；`CheckpointScope::Subagent` 物化为独立 checkpoint 仍未接通 | 已补 subagent 两事件；`CheckpointScope::Subagent` 继续作为后续 checkpoint-scope 扩展，不得误报已物化 |
...
| AG-24a / A8.5 | `agent_audit_log`、raw 访问授权（`--allow-raw`）、retention GC（transcript/stderr/findings 三窗口）、erasure 本地一致性 | `agent_audit_log_test`、raw gate、GC 不删 audit、erasure 三面一致测试 | D1/R2 delete/tombstone propagation（D1 mirror 已生效，但 erasure 不传播删除；restore 会复活） |
...
| cloud mirror delete/tombstone propagation | deferred（mirror 本身已生效：`cloud sync` upsert + `cloud restore` 读回） | local erasure 仍执行，但不传播到 D1/R2 | 删除传播落地前：restore/re-sync 会复活已本地擦除 capture，release notes 必须写明此限制 |
...
| lifecycle 事件 | `LifecycleEventKind` 11 变体（`lifecycle.rs:35`） | 缺 `SubagentStart`/`SubagentEnd`；未标 `#[non_exhaustive]` | 当前 13 变体 + `#[non_exhaustive]` 已落地；subagent events 进入 metadata，`CheckpointScope::Subagent` 独立物化仍是后续扩展 | AG-19 |
...
| AG-19 / E3 | `agent_lifecycle_event_test`：已注册（顶层 target，Cargo 自动发现） | `agent_lifecycle_event_test::invalid_hook_envelopes_are_rejected_before_checkpoint`; `agent_lifecycle_event_test::owner_claim_prevents_duplicate_checkpoint`; `agent_lifecycle_event_test::unknown_event_type_is_skipped_not_fatal`; `agent_lifecycle_event_test::kind_mismatch_still_fails_closed`; `agent_lifecycle_event_test::simultaneous_stop_race_yields_single_owner_checkpoints` | Wave 1；覆盖 owner claim、unknown event、kind mismatch、race closure、Gemini uninstall-only；SubagentStart/End 由 lifecycle/provider unit 与 Codex provider tests 守卫，当前不声明 `CheckpointScope::Subagent` 已物化 |
...
| 多 Agent | mutating `review --fix` / `investigate fix` bridge | read-only `libra review` 和 `libra investigate` 已落地；fix path 在内部 serialized AgentRuntime bridge 源码锚点与 approval/sandbox/tool-gate 测试出现前稳定返回 `LBR-AGENT-010`。 | entire `review/`,`investigate/`,`agentlaunch/` / Code fix bridge |
...
|---|---|---|---|---|---|
| 删除单个 checkpoint | 重写 `refs/libra/traces` 剔除该 checkpoint | 删除该 checkpoint 相关 OIDs | **deferred**：删除 `agent_checkpoint` D1 mirror 行或写 tombstone（mirror 已存在，删除传播未实现） | **deferred**：异步 cloud delete | 本地单删对象不可达；但 D1 mirror 行仍在，`cloud restore` 会复活——tombstone 传播落地前的已知限制 |
| 删除整个 session | 同上 + 删 `agent_session` + 删 session 目录 | 删相关 OIDs | **deferred**：删 session + checkpoint D1 mirror 行 + usage 或写 tombstone（mirror 已存在，删除传播未实现） | **deferred**：异步 cloud delete | 本地 erasure 立即一致；D1/R2 mirror 行未被删除，`cloud restore`/re-sync 会复活已擦除 session——待 tombstone 传播实现后阻止复活 |
