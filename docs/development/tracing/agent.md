...
- **trust / quarantine / provenance（AG-18 必落地）**：`agent.external_agents.enabled` 默认 `false`；未知 `libra-agent-*` 只 quarantine（`quarantined:true` + reason），不注册、不 invoke（`ERR_AGENT_EXTERNAL_AGENTS_DISABLED`）；`libra agent rpc trust` 记录 path/sha256/device/inode/mtime；hash 或 inode 变化重新 quarantine（`ERR_AGENT_PROVENANCE_REJECTED`）；built-in slug 冲突 skip-and-log（`ERR_AGENT_BUILTIN_SLUG_IMPERSONATION`）。
- protocol version mismatch fail closed（`ERR_AGENT_PROTOCOL_VERSION_MISMATCH`）；discovery 整体 fail-open=skip-and-log；`rpc invoke` 不得调用未声明 method（`ERR_AGENT_CAPABILITY_UNDECLARED`）；env/stderr/redaction 安全失败（`ERR_AGENT_IO_REDACTION_SECURITY_FAILURE`）。
...
|---|---:|---|---|---|---|
| `agent.external_agents.enabled` | `false` | bool | 是否发现并注册外部 `libra-agent-*` binary | `libra agent rpc trust` 或未来 settings 命令；也可手工配置 | false 时 discovery skip-and-log，不注册、不 invoke |
...
|---|---|---:|---|---|
| `ERR_AGENT_EXTERNAL_AGENTS_DISABLED` | external agents 未启用但请求 invoke/trust 外部 binary | 128 | 提示启用 setting 或 trust 命令 | `docs/error-codes.md` + AG-18 |
...
| A6.5 本地三 Agent 采集 smoke（plan.md §0.3） | `agent_local_capture_smoke_test`：规划 target（未注册） | `#[ignore]` + `LIBRA_RUN_LOCAL_AGENTS=1` env-gate；harness 固化 plan.md §0.3.2~§0.3.6 全流程（preflight/登录检查/hook install/真实会话/Libra 断言/卸载 smoke），test fn 命名随实现固定 | Wave local-only；新增 target 时同步 `Cargo.toml` + `tests/INDEX.md`；CI 默认必须 skip；不得以 fake fixture 替代 |
