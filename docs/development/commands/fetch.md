...
- 兼容级别：`partial`。repository/refspec、`--all`、`--depth`（Git shallow 协商路径支持；本地 Libra remote fail-closed，`LBR-REPO-002`）、`--dry-run`、`-v/--verbose`、`--porcelain`、`--tags`/`--no-tags`、`--prune`/`-p`/`--no-prune` 以及 `FETCH_HEAD` 写入与 `--append` 已公开。refspec 支持短分支、精确 `<src>:<dst>`、配置多值和双方单 `*` 通配；目标限 `refs/heads/*` 与本远程 `refs/remotes/<name>/*`，负 refspec/tag 目标仍 deferred。`--refmap`、`--atomic` 标志与 shallow 扩展参数仍未公开，但分支 refs、remote HEAD、tag 与 reflog 的落库已默认单事务原子。`--prune`/`-p` 在 fetch 完成后按 `remote prune` 的 stale 分类删除远端已不再 advertise 的 `refs/remotes/<remote>/*`（删除 + 审计 reflog 在单事务内，失败回滚；`--dry-run` 只预览不写；full-remote 范围而非 refspec 范围、远端 advertise 空 refs 时跳过——均为相对 Git 的有意收窄；本地分支、tag、`refs/remotes/<remote>/HEAD` 与其它远端不受影响）。`--prune`/`--no-prune` 为 last-one-wins toggle，并覆盖 Git 兼容的 `remote.<name>.prune` → `fetch.prune` 配置默认。
...
- 源码分层：主要实现文件为 `src/command/fetch.rs`，refspec 解析与映射集中在 `src/command/fetch/refspec.rs`。参数/子命令类型包括：`FetchArgs`；输出、错误或状态类型包括：`FetchRefUpdate`、`FetchRepositoryResult`、`FetchOutput`、`RemoteSpecErrorKind`、`FetchError`；主要执行函数包括：`execute`、`execute_safe`、`fetch_repository_safe`。
...
- 2026-07-13（plan-20260708 P1-06）：新增 `FetchRefspec` 预解析/展开层；CLI refspec 覆盖配置，多值配置驱动精确选择与目标映射，目标碰撞和缺失 exact source 在网络写入前失败。`update_references` 将所有分支目标、reflog、remote HEAD 与 tag 放进同一个 SQLite 事务；未强制非快进使整组回滚。`FETCH_HEAD` 按选择源而非变化集生成，up-to-date 仍保留行，精确 CLI refspec 为 merge candidate；fetch 不写 ORIG_HEAD。回归集中在 `compat_fetch_remote_refspec`。
...
- `-f` / `--force`：允许非 fast-forward 更新并 clobber 指向别处的本地 tag；输出对非 FF/clobber 标 `+`（porcelain）/`(forced update)`（human）。FF 判定用 `commit_is_ancestor`；未由前导 `+` refspec 或 `--force` 授权的非快进会拒绝并回滚整个 refs 事务。
...
| 类别 | 未完成项 | 当前处理 |
|---|---|---|
| 兼容差异项 | `--atomic` / `--refmap` 标志 | 原始对照：`git fetch --atomic` / `--refmap`；当前未公开独立标志。P1-06 已把正常 fetch 的 refs 默认原子化并支持基础 refspec 映射；任意目标命名空间、负 refspec 和 CLI 多 refspec 仍待扩展。 |
