...
- 2026-09-20（issues/476 WT-02）：`-M[<raw>]` 作为 `--find-renames` 的短拼写入 argv 扫描——独立 token（`-M`、`-M90%`）与短集群（`-sM90`）都记录 `FindRaw` occurrence 并改写为 clap 安全占位 `-M50`/`-sM50`；clap 声明加 `short = 'M'`（`num_args=0..=1`、`default_missing_value="50"`）。四拼写（`-M`/`--find-renames`/`--renames`/`--no-renames`）真 last-one-wins 由既有 occurrence 列表承接；阈值经共享 `diff::options::parse_rename_score` 解析。回归：`short_m_rename_spellings_are_scanned`、`short_m_takes_part_in_last_wins_ordering`、`status_test::test_status_short_m_rename_threshold_matrix`（M-RENAME M1–M6；`-M101` 按 Git/diff 语义接受，矩阵 「129」 已登记偏差）。
