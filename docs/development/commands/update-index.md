...
- 测试：`tests/command/update_index_test.rs`（cacheinfo→write-tree round-trip、`--add`、`--remove`、非法 mode/oid → 128、未跟踪路径无 `--add` → 128、非仓库 128、`--json`）；`tests/compat/write_tree_missing_object_test.rs` 覆盖未解析 cacheinfo 对象被后续写入路径拒绝。
