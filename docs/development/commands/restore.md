...
- 2026-09-04（plan-20260903 MG-00 / ADR-MG-01）：反向转换补齐——把 `160000` 指针 materialize 成目录占位时，会先删掉恰好落在该路径上的普通文件/符号链接。反向同理：source 不再声明该指针时，plain restore 会删掉那一格路径上的东西。两个方向合为一条规则——仅当索引把该路径记为**普通 tracked 内容**（可恢复）或它是目录（空占位可删、非空由 `NonEmptyWorktreeDirectory` 拦）时才允许改动；否则以 `RestoreError::SubmodulePathNotOwned`（`LBR-CONFLICT-002`）在任何写入前拒绝（`ensure_submodule_path_is_replaceable`，source/index 与 `--ours`/`--theirs` 两条预检共用）。`--merge` 渲染的是冲突标记**文件**、不会建目录占位，故不适用。另新增只读入口 `preflight_worktree_restore_to_commit`，供 `merge` 的 fast-forward 在移动 ref 之前复用同一套预检（restore 本身刻意排在指针更新之后）。
...
- 2026-09-20（issues/470 FM-01）：引入共享工作树 blob 原语 `src/utils/worktree_blob.rs`（复用 `StreamingAtomicFile` 的同目录临时文件 + rename）：普通文件按条目 mode 创建（`100755`→`0777`、`100644`→`0666`，内核应用进程 `umask`），替换已有文件时原子发布内容与权限并清除遗留可执行位；symlink 用临时链接 + rename 替换。`restore_target_to_file_typed`（含 LFS 本地缓存与下载路径）、`restore_conflict_merge`、遗留 `restore_to_file` 与 `restore_worktree*` 改为经原语写入；`worktree_mode_matches` 比较属主可执行位，mode-only 差异触发重写；删除固定 `0644/0755` 的 `apply_worktree_target_mode`。回归：`utils::worktree_blob` 单测 4 个 + 集成 8 个（restore/switch/checkout/merge/pull/worktree/clone）。
