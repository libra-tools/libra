...
| 历史编辑 | `merge`（含 P1-07b `-s ours`、`-X ours/theirs`、unrelated histories、CLI shortlog）、`rebase`（含 P1-07a autostash/exec/update-refs/fork-point）、`cherry-pick` / `cherry-pick -x`、`revert`、conflict/continue/abort 状态 | 1 | 已实现 | Runner: `cli.merge-rebase-cherry-revert-smoke`, `cli.merge-conflict-continue`, `cli.rebase-conflict-continue`; Cargo compat: `compat_noninteractive_history_controls` |
