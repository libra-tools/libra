...
| 分支与工作区切换 | `branch`、`switch`、`checkout`、detach、path checkout、远端分支可见性、`switch --guess`/`--no-guess` DWIM、未支持 switch flag 的负向路径、符号引用行为；`post-checkout` argv/show-current/already-on/逃逸阀 | 1 | 已实现 | Runner: `cli.branch-switch-checkout`; Cargo: `compat_libra_hooks_lifecycle` |
