...
| 任务 | 说明 |
|---|---|
| `gc` | 递归追踪 SQLite refs/reflogs（含 annotated-tag target）、全部 index stage、文件型 stash reflog 的每个条目与 merge/rebase held-autostash sidecar 后删除不可达 loose objects；root 或可达对象损坏/不可读时在删除前 fail-closed。被 prune 的对象在同一趟里删除其 `object_index` 目录行（PD-04——cloud sync 不再通告已删除 blob；孤儿 agent-findings blob 被回收，live-run manifest 引用的保持存活）；`--dry-run` 同时报告两个计数 |
