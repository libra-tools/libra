...
输出可通过标志调整为只显示文件名（`-l`、`-L`）、每文件匹配数量（`-c`）、行号（`-n`）、字节偏移（`-b`）或反向匹配（`-v`）。命令支持 pathspec 过滤，以将搜索限制到特定文件或目录。
...
| Perl 正则 | 拒绝（退出 129） | `-P` / `--perl-regexp` | N/A |
...
| 无效正则模式 | `LBR-CLI-003`（CliInvalidTarget） | 129 |
| 找不到修订（`--tree`） | `LBR-CLI-003`（CliInvalidTarget） | 129 |
| 未找到匹配 | `LBR-CLI-003`（CliInvalidTarget） | 129 |
| 无法读取文件（非致命） | 输出中的 warning，跳过文件 | 0 |
| 无法读取模式文件（`-f`） | 带文件路径详情的错误 | 128 |
