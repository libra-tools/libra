...
| Summary | | `--summary` | 显示创建、删除和已检测重命名文件的精简摘要（默认开启重命名检测；`diff.renames=false` 或 `--no-renames` 可关闭）。纯内容修改不产生行；不暴露纯 mode 变更。 |
...
- `--summary`（精简的 create/delete/rename 摘要；重命名默认检测、可显式关闭，不暴露纯 mode 变更）
