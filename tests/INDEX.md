...
| `compat_fetch_remote_refspec` | 1 | Guards explicit fetch source/destination mapping, configured fetch refspec selection, atomic multi-ref rollback, `remotes.default`, remote-rename tracking-ref migration, remote HEAD/symref metadata, and FETCH_HEAD/ORIG_HEAD behavior | `src/command/fetch.rs`, `src/command/fetch/refspec.rs`, `src/command/remote.rs`, `src/internal/config.rs`, `src/command/ls_remote.rs` |
