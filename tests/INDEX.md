...
| `agent_live_gate_test` | 3 | plan-20260713 live agent gate (feature `test-live-agent` + env `LIBRA_RUN_LIVE_AGENT_GATE=1`): real by-id lookups against the developer machine's actual ~/.claude and ~/.codex stores; missing stores print skipped | `src/internal/ai/observed_agents/builtin/{claude_code.rs,stable_promoted.rs}` |
