...
| `compat_noninteractive_history_controls` | 1 | Guards P1-07a rebase autostash/exec/update-refs/fork-point controls plus P1-07b merge `-s ours`, hunk-level `-X ours/theirs`, unrelated-history clean/conflict/restart/continue, and CLI shortlog persistence/toggles | `src/command/rebase.rs`, `src/command/merge.rs`, `src/command/merge_message.rs`, `src/command/stash.rs`, `src/command/maintenance.rs` |
