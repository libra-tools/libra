...
| `agent_bridge_protocol_test` | 1 | bridge protocol v1.2: 22-method allowlist including `memory.recall` and `memory.episode.record`, v1 limits (256 KiB frame, 64 in-flight, 64-event batch, page 100), initialize/major-mismatch/unknown-method/parse rejection | `src/internal/ai/agent_bridge/protocol.rs` |
...
| `agent_bridge_ingress_test` | 1 | session/event ingress: idempotent scope-bound `session.open`, closed-session reopen and process-active lifecycle, `event.append` batch ack with duplicate/digest-conflict/oversize handling, durable `session.flush`/`close`, evidence/provenance links, v1 caps fail-closed | `src/internal/ai/agent_bridge/{ingress,storage}.rs` |
...
| `fts5_capability_test` | 1 | M2-02F release-linked SQLite capability proof: external-content FTS5 create/insert, parameter-bound MATCH, fixed weighted BM25 ASC semantics, and unicode61 diacritic behavior | `src/internal/ai/memory/fts_sql.rs`, `sql/migrations/2026090702_memory_fts_search.sql` |
