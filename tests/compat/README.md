...
| `installer_https_examples_guard.rs` | installer issue #426 | every user-facing `curl ... install.sh` example inside `install.sh` uses the canonical `https://libra.tools/install.sh` URL, preventing broken retry/help snippets |
