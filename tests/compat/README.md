...
| `fetch_remote_refspec_test.rs` | P1-06 (plan-20260708) | fetch 精确 `<src>:<dst>`、`remote.<name>.fetch` 分支选择、`remotes.default`、remote rename 的 tracking-ref/config 迁移、remote HEAD / `ls-remote --symref` / FETCH_HEAD / ORIG_HEAD 元数据，以及多 ref 非快进失败的事务回滚 |
