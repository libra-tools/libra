...
| `serial_registry.rs` | plan-20260729 CT3-07 | 串行标注的成本必须是刻意的：每条仍存在的 `#[serial]` 在 `tests/SERIAL_REGISTRY.tsv` 有一行注明 lane 与理由；注册表与 `tests/SERIAL_CLASSIFY.sh` 的判定**双向**一致（守卫重跑分类器比对，杜绝手写通行证）；无 key 的 `#[serial]` 只能记为 `global`（它占的是唯一的全局车道）；分类器确定性。|
