...
| `2026090101`  | `operation_v2` | `2026090101_operation_v2.sql` (forward-only Operation v2 foundation; legacy rows remain under `legacy_*`.) |
| `2026090701`  | `memory_core` | `2026090701_memory_core{,_down}.sql` (M2-02: rebuildable Memory projections plus bounded per-root compiler job and source-observer state; FTS5 and receipt storage land in later migrations.) |
| `2026090702`  | `memory_fts_search` | `2026090702_memory_fts_search{,_down}.sql` (M2-02F: rebuildable Episode search document plus external-content FTS5 using the fixed `unicode61 remove_diacritics 2` tokenizer; non-empty rollback is refused.) |
| `2026090703`  | `context_selection_receipt` | `2026090703_context_selection_receipt{,_down}.sql` (M2-02R: shared local-only context selection receipt ledger with per-repository 30-day/10,000-row retention metadata; non-empty rollback is refused.) |
