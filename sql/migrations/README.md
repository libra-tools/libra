...
| `2026082501`  | `memory_core` | `2026082501_memory_core{,_down}.sql` (M2-02: rebuildable Memory projections plus bounded per-root compiler job and source-observer state; FTS5 and receipt storage land in later migrations.) |
| `2026082502`  | `memory_fts_search` | `2026082502_memory_fts_search{,_down}.sql` (M2-02F: rebuildable Episode search document plus external-content FTS5 using the fixed `unicode61 remove_diacritics 2` tokenizer; non-empty rollback is refused.) |
| `2026082503`  | `context_selection_receipt` | `2026082503_context_selection_receipt{,_down}.sql` (M2-02R: shared local-only context selection receipt ledger with per-repository 30-day/10,000-row retention metadata; non-empty rollback is refused.) |
