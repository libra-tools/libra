...
- `web/` (the former Next.js Code UI static export embedded into the binary) and the Publish Cloudflare Worker (`worker/`) were removed in 0.23.x; `build.rs` runs no web build, so the `LIBRA_SKIP_WEB_BUILD` / `LIBRA_PNPM` knobs no longer exist.
...
- Feature-gated layers (see `tests/INDEX.md` waves; CI job `compat-offline-core` runs the default-feature suite with L2/L3 secrets injected — step "Run tests (L1 + L2 + L3)"; `required-features` targets are excluded — plus dedicated `otlp` / `keyring` / `test-upgrade` steps; `compat-network-remotes` runs L2):
  ...
  - `--features test-live-ai` (Wave 4) is retired: the Code-era live LLM targets (`ai_agent_test`, `ai_chat_agent_test`, `ai_ollama_live_gate_test`) were removed in 0.23.x, and no `[[test]]` target requires the feature anymore.
